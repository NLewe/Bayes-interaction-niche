...
design <- model.matrix (~ as.numeric (Dim.1) + as.numeric (Dim.2) +as.numeric (Dim.3) + group  , data = y$samples)
design <- model.matrix (~  as.numeric (Dim.3) + group  , data = y$samples)
...
## boxplot - changes in AMF taxa foe change of Dim3 = 1
DAA_fit_Dim1_table %>% ggplot (aes (x = Genus , y = logFC)) + geom_violin() + geom_point()


# i.e. compare treatments
DAA_fit_Dim2 <- glmQLFTest(fit, coef = 3) 
...
DAA_fit_Dim2_table <-
  ...
  left_join (taxa_edgeR_gen) %>% 
  filter (Phylum == "Glomeromycota" ) %>% 
  add_column (PlaSpe = "Dim2") %>% ### change here a
  mutate (Sign = case_when(PValue<=0.05 ~ "sign.", PValue > 0.05 ~ "ns")) %>%  
  arrange (PValue)

## boxplot - changes in AMF taxa foe change of Dim3 = 1
DAA_fit_Dim2_table %>% ggplot (aes (x = Genus , y = logFC)) + geom_violin() + geom_point()

# i.e. compare treatments
DAA_fit_Dim3 <- glmQLFTest(fit, coef = 4) 
...
## boxplot - changes in AMF taxa foe change of Dim3 = 1
DAA_fit_Dim3_table %>% ggplot (aes (x = Genus , y = logFC)) + geom_violin() + geom_point()
...
DAA_fit_AchMil <- glmQLFTest(fit, coef = 5) 

DAA_fit1_table <-
  DAA_fit_AchMil$table %>%  
  as_tibble (rownames = "ASV_ID") %>% 
  left_join (taxa_edgeR_gen) %>% 
  filter (Phylum == "Glomeromycota" ) %>% 
...
DAA_fit_AgrCap <- glmQLFTest(fit, coef = 6) 

DAA_fit3_table <-
  DAA_fit_AgrCap$table %>%  
  as_tibble (rownames = "ASV_ID") %>% 
  left_join (taxa_edgeR_gen) %>% 
  filter (Phylum == "Glomeromycota" ) %>% 
...
DAA_fit4_vsC <- glmQLFTest(fit, coef = 7) 
...
DAA_fit5_vsC <- glmQLFTest(fit, coef = 8) 
...
DAA_fit6_vsC <- glmQLFTest(fit, coef = 9) 
...
DAA_fit7_vsC <- glmQLFTest(fit, coef = 10) 
...
DAA_fit8_vsC <- glmQLFTest(fit, coef = 11) 
...
DAA_fit9_vsC <- glmQLFTest(fit, coef = 12) 
...
# add genus labels
DAA_Dim_all <- 
  bind_rows (DAA_fit_Dim1_table, DAA_fit_Dim2_table, DAA_fit1_table, DAA_fit_Dim3_table, DAA_fit3_table, DAA_fit4_table,
             DAA_fit5_table, DAA_fit6_table, DAA_fit7_table, DAA_fit8_table, DAA_fit9_table)  %>%  
...
plotDAA_gen <-
  DAA_Dim_all %>% #filter (PlaSpe == "Dim3")  %>% 
