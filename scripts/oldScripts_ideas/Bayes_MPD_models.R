...
mpd_fit0 <- brm(
  ...
  family = gaussian(),
  data2 = list(A = A),
  prior = priorB,  sample_prior = TRUE, chains = 4, cores = 8,
...
loo_compare (mpd_fit0, mpd_fit01, Gen_samples_fit05)
...
plot (conditional_effects(mpd_fit01, effects = "MPD:DW_roots", ndraws = 1000, spaghetti = F, mean = T, prob = 0.5), points = T)
