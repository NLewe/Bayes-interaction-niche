...
new_sampledata_gen <- 
  ...
  as_tibble (rownames= "sampleID") %>% 
  left_join (test2) %>% 
  mutate (GenBins = replace_na(GenBins, "control")) %>% 
  # mutate (Gen = case_when(0<RelGenSpec & RelGenSpec < 0.1 ~ "gen01",
  #                         0.1< RelGenSpec & RelGenSpec <0.2 ~ "gen02",
  #                         0.2< RelGenSpec & RelGenSpec <0.3 ~ "gen03",
  #                         0.3< RelGenSpec & RelGenSpec <0.4 ~ "gen04",
  #                         0.4< RelGenSpec & RelGenSpec <0.5 ~ "gen05",
  #                         0.5< RelGenSpec & RelGenSpec <0.6 ~ "gen06",
  #                         0.6< RelGenSpec & RelGenSpec <0.7 ~ "gen07",
  #                         0.7< RelGenSpec & RelGenSpec <0.8 ~ "gen08",
  #                         0.8< RelGenSpec & RelGenSpec <0.9 ~ "gen09",
  #                         0.9< RelGenSpec & RelGenSpec <1 ~ "gen1.0" , 
  #                         is.na (RelGenSpec) ~ "control")   ) %>% 
    data.frame (row.names = "sampleID") %>% 
  sample_data()
...
# DAA#####
## Grouping : values of generalism versus control
group = factor (sample_data (ps_edgeR_relGenSpec)$GenBins)
...
y$samples$GenBins <- factor (sample_data (ps_edgeR_relGenSpec)$GenBins)
...
#   B1 =Bin1 -control, B2 = Bin2-control, 
#   B3 =Bin3 -control, B4 = Bin4-control,
#   B5 =Bin5 -control, B6 = Bin6-control, 
#   B7 = Bin7 -control, B8 = Bin8 -control,
...
DAA_fitBin1_vsC <- glmQLFTest(fit, coef = 2 ) 


summary(decideTests(DAA_fitBin1_vsC))

#Soil Bin 1 results####
# Ach Mil data soil ##
DAA_fitBin1_table <-
  DAA_fitBin1_vsC$table %>%  
  as_tibble (rownames = "ASV_ID") %>% 
  left_join (taxa_edgeR_gen) %>% 
  filter (Phylum == "Glomeromycota" ) %>% 
  add_column (Gen = "Bin1") %>% ### change here 
...
#compare the coefficients of the glm 
# i.e. compare treatments
#DAA_fitBin2_vsC <- glmQLFTest(fit, contrast = my_contrasts [,"B2"]) ## for design matrix ~ 0+ group
DAA_fitBin2_vsC <- glmQLFTest(fit, coef = 3) 

summary(decideTests(DAA_fitBin2_vsC))

#Soil Bin 2 results####
# Ach Mil data soil ##
DAA_fitBin2_table <-
  DAA_fitBin2_vsC$table %>%  
  as_tibble (rownames = "ASV_ID") %>% 
  left_join (taxa_edgeR_gen) %>% 
  filter (Phylum == "Glomeromycota" ) %>% 
  add_column (Gen = "Bin2") %>% ### change here 
  mutate (Sign = case_when(PValue<=0.05 ~ "sign.", PValue > 0.05 ~ "ns")) %>%  
  arrange (PValue)
...
#compare the coefficients of the glm 
# i.e. compare treatments
DAA_fitBin3_vsC <- glmQLFTest(fit, coef = 4) 

summary(decideTests(DAA_fitBin3_vsC))

#Soil Bin 3 table ####
# Ach Mil data soil ##
DAA_fitBin3_table <-
  DAA_fitBin3_vsC$table %>%  
  as_tibble (rownames = "ASV_ID") %>% 
  left_join (taxa_edgeR_gen) %>% 
  filter (Phylum == "Glomeromycota" ) %>% 
  add_column (Gen = "Bin3") %>% ### change here 
  mutate (Sign = case_when(PValue<=0.05 ~ "sign.", PValue > 0.05 ~ "ns")) %>%  
  arrange (PValue)
...
DAA_fitBin4_vsC <- glmQLFTest(fit, coef = 5) 

summary(decideTests(DAA_fitBin4_vsC))

#Soil PlaSpe results####
# Ach Mil data soil ##
DAA_fitBin4_table <-
  DAA_fitgen06_vsC$table %>%  
  as_tibble (rownames = "ASV_ID") %>% 
  left_join (taxa_edgeR_gen) %>% 
  filter (Phylum == "Glomeromycota" ) %>% 
  add_column (Gen = "Bin4") %>% ### change here 
  mutate (Sign = case_when(PValue<=0.05 ~ "sign.", PValue > 0.05 ~ "ns")) %>%  
  arrange (PValue)
...
#compare the coefficients of the glm 
# i.e. compare treatments
DAA_fitBin5_vsC <- glmQLFTest(fit, coef = 6) 

summary(decideTests(DAA_fitBin5_vsC))

#Soil Bin 5 results####
# Ach Mil data soil ##
DAA_fitBin5_table <-
  DAA_fitBin5_vsC$table %>%  
  as_tibble (rownames = "ASV_ID") %>% 
  left_join (taxa_edgeR_gen) %>% 
  filter (Phylum == "Glomeromycota" ) %>% 
  add_column (Gen = "Bin5") %>% ### change here 
  mutate (Sign = case_when(PValue<=0.05 ~ "sign.", PValue > 0.05 ~ "ns")) %>%  
  arrange (PValue)
...
#compare the coefficients of the glm 
# i.e. compare treatments
DAA_fitBin6_vsC <- glmQLFTest(fit, coef = 7) 

summary(decideTests(DAA_fitBin6_vsC))

#Soil PlaSpe results####
# Ach Mil data soil ##
DAA_fitBin6_table <-
  DAA_fitBin6_vsC$table %>%  
  as_tibble (rownames = "ASV_ID") %>% 
  left_join (taxa_edgeR_gen) %>% 
  filter (Phylum == "Glomeromycota" ) %>% 
  add_column (Gen = "Bin6") %>% ### change here 
  mutate (Sign = case_when(PValue<=0.05 ~ "sign.", PValue > 0.05 ~ "ns")) %>%  
  arrange (PValue)
...
#compare the coefficients of the glm 
# i.e. compare treatments
DAA_fitBin7_vsC <- glmQLFTest(fit, coef = 8) 

summary(decideTests(DAA_fitBin7_vsC))

#Soil PlaSpe results####
# Ach Mil data soil ##
DAA_fitBin7_table <-
  DAA_fitBin7_vsC$table %>%  
  as_tibble (rownames = "ASV_ID") %>% 
  left_join (taxa_edgeR_gen) %>% 
  filter (Phylum == "Glomeromycota" ) %>% 
  add_column (Gen = "Bin7") %>% ### change here 
  mutate (Sign = case_when(PValue<=0.05 ~ "sign.", PValue > 0.05 ~ "ns")) %>%  
  arrange (PValue)
...
#compare the coefficients of the glm 
# i.e. compare treatments
DAA_fitBin8_vsC <- glmQLFTest(fit, coef = 9) 

summary(decideTests(DAA_fitBin8_vsC))

#Soil PlaSpe results####
# Ach Mil data soil ##
DAA_fitBin8_table <-
  DAA_fitBin8_vsC$table %>%  
  as_tibble (rownames = "ASV_ID") %>% 
  left_join (taxa_edgeR_gen) %>% 
  filter (Phylum == "Glomeromycota" ) %>% 
  add_column (Gen = "Bin8") %>% ### change here 
  mutate (Sign = case_when(PValue<=0.05 ~ "sign.", PValue > 0.05 ~ "ns")) %>%  
  arrange (PValue)
...
# add genus label
DAA <- bind_rows (DAA_fitBin1_table, DAA_fitBin2_table, DAA_fitBin3_table, DAA_fitBin4_table,
       DAA_fitBin5_table, DAA_fitBin6_table, DAA_fitBin7_table, DAA_fitBin8_table) %>%  
