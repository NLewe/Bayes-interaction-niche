...
test2 <- 
  RelGen_E1_E2_sample %>%  
  filter (Exp =="E2") %>% 
  select (-PlantSpeciesfull, - PlaSpe, -Exp, -PlantFamily) %>%  
...
Gen_table  <-  (new_sampledata_gen$Gen)
...
#group = factor (sample_data (ps_edgeR_relGenSpec)$)

## prepare edgeR object y
y <- DGEList(counts = df, group = Gen_table)
...
X <- poly (new_sampledata_gen$RelGenSpec, degree =3)
design <- model.matrix (~ as.numeric (RelGenSpec)  , data = y$samples)
...
y$samples$RelGenSpec <- factor (sample_data (ps_edgeR_relGenSpec)$RelGenSpec)
...
colnames(design) <- levels(y$samples$Gen)
...
DAA_fit1vs066 <- glmQLFTest(fit, coef = 2) 

summary(decideTests(DAA_fit1vs066))
...
tab <- as.data.frame (topTags(DAA_fit1vs066, n = 310))
...
logCPM.fit <- cpm(DAA_fit1vs066, log=TRUE)
 ...
 for(i in 1:25) {
   ...
   plot(new_sampledata_gen$RelGenSpec, logCPM.obs.i, ylab="log-CPM", main=ASV_ID, pch=16)
  lines(new_sampledata_gen$RelGenSpec, logCPM.fit.i, col="red", lwd=2)
   }
